/**
 ...
 @param nameTemplateId optional type template number. Set this if you want just one of the options.
 examples:
 ...
 */
import { changeImageOfNextItemChatMessage, renameNextChatMessageItemName } from '../utils/chat-utils.js'

export const animefyNextAttack = (nameTemplateId) => {
  const newName = animeAttackNameGen(nameTemplateId)
  renameNextChatMessageItemName(newName)
  const newImg = animeImageGen(nameTemplateId)
  changeImageOfNextItemChatMessage(newImg)
}

function animeImageGen (nameTemplateId) {
  const type = nameTemplateId || pick([1, 2])
  if (type === 1) {
...
function animeAttackNameGen (nameTemplateId) {
  const type = nameTemplateId || pick([1, 2, 3, 4])
  if (type === 1) {
    // e.g. Armageddon Barrage
    return pick(nm2) + ' ' + pick(nm3)
  } else if (type === 2) {
    // e.g. Majestic Lion Burst
    return pick(nm1) + ' ' + pick(nm2) + ' ' + pick(nm3)
  } else if (type === 3) {
    // e.g. Watchful Shark, Splitting Jackal
    ...
    // e.g. Whirling Crush of Darling Star
    return pick(nm6) + ' ' + pick(nm3) + ' of ' + pick(nm4) + ' ' + pick(nm5)
  } else {
    // error
    console.error('animeAttackNameGen expected 0 <= i <= 4')
