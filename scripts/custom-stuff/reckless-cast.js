...
should be called by a macro per spell level, e.g.:
  ...
  requires assumes BetterRolls5e is installed, and may recursively call itself rarely.
  ...
  for (const roll of rolls) {
	...
	<header class="card-header flexrow red-header">
      ...
      let target = (data.target && data.target.type) ? i18n('Target: ')
          .concat(i18n(dnd5e.targetTypes[data.target.type])) +
        ((data.target.units) && (data.target.units !== 'none') ? ' (' + data.target.value + ' ' +
          i18n(dnd5e.distanceUnits[data.target.units]) + ')' : '') : null
      let duration = (data.duration && data.duration.units) ? (data.duration.value ? data.duration.value + ' ' : '') +
        i18n(dnd5e.timePeriods[data.duration.units]) : null
      const properties = [
      ...
      const randomId = Math.random().toString(36).substring(2)
      // add ID to the image
      const spellCopyName = spellCopy.name.replace('&', '&amp;')
        .replace('<', '&lt;')
        .replace('>', '&gt;')
        .replace('\'', '&#x27;')
        .replace('"', '&quot;')
